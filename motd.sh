...
mpstat | echo "   * CPU: $(awk '$12 ~ /[0-9.]+/ { print 100 - $12"%" }') ($(grep ^cpu\\scores /proc/cpuinfo | uniq | awk '{print $4}')c/$(grep -c ^processor /proc/cpuinfo)t @ $(echo "scale=2; ${_delta_CPUCLOCK%.*} / 1000" | bc)GHz)"
